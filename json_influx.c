...
void add_flow_influx(uuid_t flowid, struct estats_connection_info *conn) {
	...
	*influx_data = '\0';
	...
	length = snprintf(NULL, 0, ",type=flowdata,group=%s,domain=%s,dtn=%s,flow=%s value=", 
			  options.group, options.domain_name, options.dtn_id, flowid_char);
	length++;
 ...
 */
/* void add_time(uuid_t uuid, struct estats_nl_client *cl, int cid, char *time_marker) { */
...
/*         json_object *pnt_value = json_object_new_int64(timestamp); */
...
void add_time(uuid_t uuid, struct estats_nl_client *cl, int cid, char *time_marker) {
	...
	length = snprintf(NULL, 0, "%s,type=flowdata,group=%s,domain=%s,dtn=%s,flow=%s value=%"PRIu64"\n", 
			  time_marker, options.group, options.domain_name, 
...
/* 	length = snprintf(NULL, 0, "flowmetrics,domain=%s,dtn=%s,flow=%s",  */
/* 			  options.domain_name, options.dtn_id, flowid_char); */
/* 	length++; */
/* 	name_str = malloc(length * sizeof(char)); */
/* 	snprintf(name_str, length, "flowmetrics,domain=%s,dtn=%s,flow=%s",  */
/* 		 options.domain_name, options.dtn_id, flowid_char); */
...
void read_metrics (struct ConnectionHash *flow, struct estats_nl_client *cl) {
	...
	*influx_data = '\0';
	...
	for (i = 0; i < esdata->length; i++) {
		...
		size = snprintf(NULL, 0, "%s%s%s%d\n", estats_var_array[i].name, tag_str, estats_val, flow->lastpoll) + 1;
		...
		temp_str = malloc(size);
		snprintf(temp_str, size, "%s%s%s%d\n", estats_var_array[i].name, tag_str, estats_val, flow->lastpoll);
